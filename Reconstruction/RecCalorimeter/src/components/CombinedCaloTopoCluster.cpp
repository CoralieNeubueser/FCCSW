...
StatusCode CombinedCaloTopoCluster::initialize() {
  ...
  m_decoderECal = std::shared_ptr<DD4hep::DDSegmentation::BitField64>(m_geoSvc->lcdd()->readout(m_ECalReadoutName).segmentation().segmentation()->decoder());
  m_decoderHCal = std::shared_ptr<DD4hep::DDSegmentation::BitField64>(m_geoSvc->lcdd()->readout(m_HCalReadoutName).segmentation().segmentation()->decoder());
...
StatusCode CombinedCaloTopoCluster::execute() {
  ...
  if (m_noiseECal)
    CombinedCaloTopoCluster::findingSeedsWNoise(ECalCells, m_seedThr_ECal, firstSeedsECal, m_allCellsECal, *m_noiseToolECal);
  else 
    CombinedCaloTopoCluster::findingSeeds(ECalCells, m_seedThr_ECal, firstSeedsECal, m_allCellsECal);

  if (m_noiseHCal)
    CombinedCaloTopoCluster::findingSeedsWNoise(HCalCells, m_seedThr_HCal, firstSeedsHCal, m_allCellsHCal, *m_noiseToolHCal);
  else
  ...
  if(m_noiseECal)
    CombinedCaloTopoCluster::buildingProtoClusterWNoise(m_neighbourThr_ECal, NeighboursMapECal, firstSeedsECal, m_allCellsECal, preClusterCollectionECal, *m_noiseToolECal);
  else
  ...
  for (auto i : preClusterCollectionECal) {
    ...
    if(clusterToBeMerged){
      ...
      debug() << "Eta-/+rms:     " << meanEtaLastLayer-2*(m_HCalSegmentation->gridSizeEta()) << "/" << meanEtaLastLayer+2*(m_HCalSegmentation->gridSizeEta()) << endmsg;
      debug() << "Phi-/+rms:     " << meanPhiLastLayer-2*(m_HCalSegmentation->gridSizePhi()) << "/" << meanPhiLastLayer+2*(m_HCalSegmentation->gridSizePhi()) << endmsg;
  ...
  if(m_noiseHCal)
    CombinedCaloTopoCluster::buildingProtoClusterWNoise(m_neighbourThr_HCal, NeighboursMapHCal, firstSeedsHCal, m_allCellsHCal, preClusterCollectionHCal, *m_noiseToolHCal);  
  else 
  ...
  for (auto i : preClusterCollectionHCal) {
    ...
    if(clusterToBeMerged){
      ...
      // get cluster position in eta/phi
      DD4hep::Geometry::Position clusterPos(cluster.position().x, cluster.position().x, cluster.position().x);
	...
	if (eta >= clusterECalEta.second.first && eta <= clusterECalEta.second.second){
	  // Check Phi region 
	  if (phi >= mergeClustersPhiMap[clusterECalEta.first].first && phi <= mergeClustersPhiMap[clusterECalEta.first].second){
	    debug() << "Found cluster to merge!" << endmsg;
	    // Adding all ECal cells to the cluster
	    for (auto itECalCells = mergeClustersPhiMap.find(clusterECalEta.first)->first.hits_begin(); itECalCells != mergeClustersPhiMap.find(clusterECalEta.first)->first.hits_end(); ++itECalCells){
	      cluster.addhits(*itECalCells);
	    }
	    // Setting new cluster poperties
	    // Energy weighted positions and energy sum
	    clusterCore.position.x = ((clusterCore.position.x * clusterCore.energy) + (clusterECalEta.first.core().position.x * clusterECalEta.first.core().energy))/2;
	    clusterCore.position.y = ((clusterCore.position.y * clusterCore.energy) + (clusterECalEta.first.core().position.y * clusterECalEta.first.core().energy))/2;
	    clusterCore.position.z = ((clusterCore.position.z * clusterCore.energy) + (clusterECalEta.first.core().position.z * clusterECalEta.first.core().energy))/2;
...
void CombinedCaloTopoCluster::buildingProtoClusterWNoise(double neighbourThr,
  ...
  for(auto itSeed = seeds.begin(); itSeed != seeds.end(); ++itSeed) {
	...
	debug() << "Seed is not found in cell list! This should not happen!" << endmsg;
      ...
      std::vector<uint64_t> N1 = CombinedCaloTopoCluster::searchForNeighboursWNoise(seedId, clusterId, neighboursMap, neighbourThr, allCells, clusterOfCell, preClusterCollection, aNoiseTool);
      N2[0] = N1;
      debug() << "Found " << N2[0].size() << " neighbours.." << endmsg;
      for (int it=1; it<100; it++){
	for (auto& id : N2[it-1]) {
	    N2[it] = CombinedCaloTopoCluster::searchForNeighboursWNoise(id, clusterId, neighboursMap, neighbourThr, allCells, clusterOfCell, preClusterCollection, aNoiseTool);
	    if (N2[it].size() == 0){ // try with neighbour threshold=0
	      double thr = 0.;
	      N2[it] = CombinedCaloTopoCluster::searchForNeighboursWNoise(id, clusterId, neighboursMap, thr, allCells, clusterOfCell, preClusterCollection, aNoiseTool);
	      if (N2[it].size() == 0)
		break;
	    }	      
...
void CombinedCaloTopoCluster::buildingProtoCluster(double neighbourThr,
  ...
  for(auto itSeed = seeds.begin(); itSeed != seeds.end(); ++itSeed) {
	...
	debug() << "Seed is not found in cell list! This should not happen!" << endmsg;
      ...
      //int maxN = neighboursMap.find(seedId)->second.size();
      std::vector< std::vector<uint64_t> > N2(100);
      std::vector<uint64_t> N1 = CombinedCaloTopoCluster::searchForNeighbours(seedId, clusterId, neighboursMap, neighbourThr, allCells, clusterOfCell, preClusterCollection);
      N2[0] = N1;
      debug() << "Found " << N2[0].size() << " neighbours.." << endmsg;
      for (int it=1; it<100; it++){
	for (auto& id : N2[it-1]) {
	  N2[it] = CombinedCaloTopoCluster::searchForNeighbours(id, clusterId, neighboursMap, neighbourThr, allCells, clusterOfCell, preClusterCollection);
	  if (N2[it].size() == 0){ // try with neighbour threshold=0
	    double thr = 0.;
	    N2[it] = CombinedCaloTopoCluster::searchForNeighbours(id, clusterId, neighboursMap, thr, allCells, clusterOfCell, preClusterCollection);
	    if (N2[it].size() == 0)
	      break;
	  }
	  debug() << "Found " << N2[it].size() << " more neighbours.." << endmsg;
...
std::vector<uint64_t> CombinedCaloTopoCluster::searchForNeighboursWNoise(const uint64_t id, 
									 uint& clusterNum, 
									 const std::unordered_map<uint64_t, std::vector<uint64_t> > neighboursMap, 
									 double noiseSigma, 
									 std::map<uint64_t, fcc::CaloHit>& allCells, 
  ...
  for(auto& itr : search->second) {
    ...
    if (itAllCells != allCells.end()){
      ...
      noiseSigma = aNoiseTool.getNoiseConstantPerCell(neighbouringCell.cellId()); // if noise is added, retrieve the cell noise level
	...
	debug() << "This neighbour was found in cluster " << clusterNumToMerge << ", mark cluster " << clusterNum << " to be merged!" << endmsg;
  ...
  if (rmCluster){
    debug() << "Assigning all cells ( " << preClusterCollection.find(clusterNum)->second.size() <<  " ) to Cluster " << addFoundNeighboursToCluster << " with ( " << preClusterCollection.find(addFoundNeighboursToCluster)->second.size() << " ). " << endmsg;
...
std::vector<uint64_t> CombinedCaloTopoCluster::searchForNeighbours(const uint64_t id, 
								   uint& clusterNum, 
								   const std::unordered_map<uint64_t, std::vector<uint64_t> > neighboursMap, 
								   double noiseSigma, 
								   std::map<uint64_t, fcc::CaloHit>& allCells, 
