...
StatusCode SplitClusters::execute() {
  ...
  if (int(totEnergyBefore)!=int(totEnergyAfter))
    error() << "After cluster splitting, energy ( " << totEnergyAfter << " ) is not what is was before ( " << totEnergyBefore << " )." << endmsg;
  if (totCellsBefore!=totCellsAfter)
    error() << "After cluster splitting, cells ( " << totCellsAfter << " ) is not what is was before ( " << totCellsBefore << " )." << endmsg;
